...
import numpy as np
import argparse
from ALE_parsing import Annotation, Cluster
from COUNT_parsing import *
import sys
import glob
...
parser.add_argument("-e", "--count_table", required=True,
                    help="name of the Count Posteriors output table")
...
def main(args):
    ...
    for f in glob.glob(args.faas + "/*"):
        try:
            ...
            print("could not find COUNT results for cluster {}".format(f), file=sys.stderr)

    counts = Counts(args.count_table)

    name2nodes = {}
    for node in set(args.ancestor_nodes + args.node_pairs):
        n = Node(node, annotation, counts, name2cluster, args.threshold)
        name2nodes[n.name] = n
